...
const validateUser = asyncHandler(async (req, res) => {
  try {
    ...
    if (firstname && (firstname.trim() === "" || firstname.trim().length < 3)) {
      ...
      !/^[^\s\d!@#$%^&*(),.?":{}|<>_+=\[\\;\]`~]*[a-zA-ZÀ-ÖØ-öø-ÿ-' ]+[^\s!@#$%^&*(),.?":{}|<>_+=\[\\;\]`~]*$/.test(
        firstname
      )
    ) {
    ...
    // Email Validation
    if (email && email.trim() === "") {
      response.fnameStatus = "Please enter email";
    } else if (
      !/^[^\s@]+@(?!.*\.[^\s@]+)[a-zA-Z\d-]+\.(com|org|in|net|edu|gov|co|uk)$/.test(
        email
      )
    ) {
    ...
    if (
      mobile &&
      (mobile.trim() === "" || mobile.length < 10 || !/^\d{10}$/.test(mobile))
    ) {
...
const verifyOtp = asyncHandler(async (req, res) => {
  try {
    ...
    if (hashedOtp === uotp) {
      ...
      res.render("otp", {
        message: req.flash("otp", "otp is incorrect"),
        email: email,
      });
    } else {
